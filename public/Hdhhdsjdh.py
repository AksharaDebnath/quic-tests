...
def generate_monthly_data(client_data):
    ...
    # Loop through client data rows
    for i in range(len(client_data)-1, -1, -1):
        current_row = client_data.iloc[i]
        next_row = client_data.iloc[i-1] if i > 0 else None

        # Determine the current date and next date boundaries
        current_date = current_row['snap_dt']

        if next_row is None:
            # Scenario 1: Most recent snap date (goes up to current month)
            last_date = pd.Timestamp(datetime.now().strftime('%Y-%m-01'))
        else:
            # Scenario 2: Use offset between two snap dates
            last_date = next_row['snap_dt'] - pd.DateOffset(months=1)
