...
for clnt_no in df['clnt_no'].unique():
    ...
    # Add the most recent assessment date to the expanded_df
    ...
    for current_date in snap_dates[1:]:
        # Add the assessment date row in the correct spot
        assessment_row = client_data[client_data['snap_dt'] == current_date].copy()
        expanded_df = pd.concat([expanded_df, assessment_row], ignore_index=True)
...
# Reorder the columns to move month_year to the left
cols = ['month_year'] + [col for col in expanded_df.columns if col != 'month_year']
